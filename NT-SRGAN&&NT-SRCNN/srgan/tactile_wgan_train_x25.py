...
## --- 路径 + debug --- ##
TRAIN_NAME = "srgan_dataset_x2_lay_num_6"
...
cuda_device_num = int(sys.argv[1])
...
epochs = 300
weight_decay = 1e-3
GNet = SRCNN_MSRN(feature_layers_num=6, scale_factor=scale_factor, is_init=False).cuda()       
DNet = Dis_Net(LRB_layer_num=3, scale_factor=scale_factor,is_init=False).cuda()
is_nor = False  
scale_value = 1000
scale_value_z = 1000
scale_value_xy = 1000
...
# 加载训练数据
dataset_file = root_path + 'dataset/TSR_data_x2/train/'              # 原始数据集
