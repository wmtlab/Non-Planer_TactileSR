...
class RegTactileData():
    ...
    def plotRegData2D(self, data_x, data_y, data_z,):
        ...
        ax_1.imshow(data_x, cmap=cmap)
        ax_2.imshow(data_y,  cmap=cmap)
        ax_3.imshow(data_z,  cmap=cmap)
    ...
    def maxminNormalization(self, seq_data_x, seq_data_y, seq_data_z):
        ...
        # seq_data_x, seq_data_y, seq_data_z = seq_data_x/100, seq_data_y/100, seq_data_z/200
        # return self.tanh(seq_data_x),self.tanh(seq_data_y),self.sigmoid(seq_data_z)
    ...
    def regData(self, data):
        ...
        for i in range(sample_num_x):
            for j in range(sample_num_x):
                # 将一次（4，4）的触觉数据放到（sample_num_x * 4， sample_num_y * 4）上面
    ...
    def smoothPattern(self, data_x, data_y, data_z, smoth_method=2):##修改了滤波器
        # 1 : 高斯滤波 2：双边滤波
    ...
    def saclePattern(self, data_x, data_y, data_z, scale_num=500):
